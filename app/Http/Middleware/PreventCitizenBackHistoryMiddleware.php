    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($request->session()->has('logged_in')) {
            if (time() - $request->session()->get('last_activity') > 3600) { // 30 minutes
                ...
                $request->session()->put('csrf_token_old', csrf_token());
                $request->session()->put('csrf_token_new', csrf_token());
                $request->session()->put('csrf_token_old_new', csrf_token());
                $request->session()->put('csrf_token_new_old', csrf_token());
                $request->session()->put('csrf_token_old_new_old', csrf_token());
                $request->session()->put('csrf_token_new_old_new', csrf_token());
                $request->session()->put('csrf_token_old_new_old_new', csrf_token());

                return redirect()->route('frontend.citizen.login')->with('error', 'Your session has expired. Please log in again.');
            } else {
