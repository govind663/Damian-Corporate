...
<div class="pd-ltr-20 xs-pd-20-10">
    <div class="min-height-200px">
        ...
        <form method="POST" action="{{ route('project.update', $project->id) }}" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="pd-20 card-box mb-30">
                ...
                <div class="form-group row mt-3">
                    ...
                    <div class="col-sm-4 col-md-4">
                        <input type="text" name="slug" id="slug" class="form-control @error('slug') is-invalid @enderror" value="{{ $project->slug }}" readonly placeholder="Enter Project Slug.">
                        @error('slug')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="col-sm-4 col-md-4">
                        <select name="category_id" id="category_id" class="custom-select2 form-control @error('category_id') is-invalid @enderror">
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" {{ $project->category_id == $category->id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                            <span class="invalid-feedback" role="alert">
                ...
                <div class="form-group row mt-3">
                    ...
                    <div class="col-sm-4 col-md-4">
                        ...
                        @if(!empty($project->project_image))
                            <img src="{{ asset('/damian_corporate/project/project_image/' . $project->project_image) }}" alt="Banner Image" style="width: 100%; height: auto;">
                        @endif
                    </div>
                </div>
