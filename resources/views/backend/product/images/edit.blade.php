...
@section('title')
Damian Corporate | Edit Product Image
@endsection
...
<div class="pd-ltr-20 xs-pd-20-10">
    <div class="min-height-200px">
        ...
        <form method="POST" action="{{ route('product-image.update', $productImage->id) }}" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <input type="hidden" name="product_id" value="{{ $productImage->id }}">
            ...
            <div class="pd-20 card-box mb-30">
                <div class="form-group row mt-3">
                    ...
                    <div class="col-sm-4 col-md-4">
                        <select name="product_id" id="product_id" class="custom-select2 form-control @error('product_id') is-invalid @enderror">
                            ...
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" {{ $productImage->product_id == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                            @endforeach
                        </select>
                        @error('product_id')
                    ...
                    <div class="col-sm-4 col-md-4">
                        <input type="file" onchange="agentPreviewFile()" accept=".png, .jpg, .jpeg, .webp" name="product_other_images" id="product_other_images" class="form-control @error('product_other_images') is-invalid @enderror" value="{{ $productImage->product_other_images }}" placeholder="Upload Product Image.">
                        ...
                        @if($productImage->product_other_images)
                            <img src="{{ asset('/damian_corporate/product/product_other_images/' . $productImage->product_other_images) }}" alt="{{ $productImage->product_other_images }}" style="width: 100%; height: 100px !important;">
                        @endif
                    </div>
                </div>
